...
class AgentList extends Agent
{
    ...
    public $FormName = "fagentlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "AgentList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "AgentAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "AgentDelete";
        $this->MultiUpdateUrl = "AgentUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->IdAgent->AdvancedSearch->toJson(), ","); // Field IdAgent
        $filterList = Concat($filterList, $this->MatricAgent->AdvancedSearch->toJson(), ","); // Field MatricAgent
        $filterList = Concat($filterList, $this->NomAgent->AdvancedSearch->toJson(), ","); // Field NomAgent
        $filterList = Concat($filterList, $this->PostnomAgent->AdvancedSearch->toJson(), ","); // Field PostnomAgent
        $filterList = Concat($filterList, $this->PrenomAgent->AdvancedSearch->toJson(), ","); // Field PrenomAgent
        $filterList = Concat($filterList, $this->SexeAgent->AdvancedSearch->toJson(), ","); // Field SexeAgent
        $filterList = Concat($filterList, $this->EtatCivilAgent->AdvancedSearch->toJson(), ","); // Field EtatCivilAgent
        $filterList = Concat($filterList, $this->TelephoneAgent->AdvancedSearch->toJson(), ","); // Field TelephoneAgent
        $filterList = Concat($filterList, $this->Motsecret->AdvancedSearch->toJson(), ","); // Field Motsecret
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fagentsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Motsecret
        $this->Motsecret->AdvancedSearch->SearchValue = @$filter["x_Motsecret"];
        $this->Motsecret->AdvancedSearch->SearchOperator = @$filter["z_Motsecret"];
        $this->Motsecret->AdvancedSearch->SearchCondition = @$filter["v_Motsecret"];
        $this->Motsecret->AdvancedSearch->SearchValue2 = @$filter["y_Motsecret"];
        $this->Motsecret->AdvancedSearch->SearchOperator2 = @$filter["w_Motsecret"];
        $this->Motsecret->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->MatricAgent;
        $searchFlds[] = &$this->NomAgent;
        $searchFlds[] = &$this->PostnomAgent;
        $searchFlds[] = &$this->PrenomAgent;
        $searchFlds[] = &$this->SexeAgent;
        $searchFlds[] = &$this->EtatCivilAgent;
        $searchFlds[] = &$this->TelephoneAgent;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Motsecret); // Motsecret
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"agent\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"agent\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"agent\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fagentlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fagentlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->IdAgent->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"agent\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fagentlist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("IdAgent", $this->createColumnOption("IdAgent"));
            $option->add("MatricAgent", $this->createColumnOption("MatricAgent"));
            $option->add("NomAgent", $this->createColumnOption("NomAgent"));
            $option->add("PostnomAgent", $this->createColumnOption("PostnomAgent"));
            $option->add("PrenomAgent", $this->createColumnOption("PrenomAgent"));
            $option->add("SexeAgent", $this->createColumnOption("SexeAgent"));
            $option->add("EtatCivilAgent", $this->createColumnOption("EtatCivilAgent"));
            $option->add("TelephoneAgent", $this->createColumnOption("TelephoneAgent"));
            $option->add("Motsecret", $this->createColumnOption("Motsecret"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fagentsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fagentsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fagentlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_agent", "data-rowtype" => ROWTYPE_ADD]);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fagentsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
