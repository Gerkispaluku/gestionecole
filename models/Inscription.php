...
class Inscription extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // IdInscription
        $this->IdInscription = new DbField(
            $this, // Table
            'x_IdInscription', // Variable name
            'IdInscription', // Name
            '`IdInscription`', // Expression
            '`IdInscription`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`IdInscription`', // Virtual expression
            false, // Is virtual
        ...
        $this->IdInscription->InputTextType = "text";
        $this->IdInscription->IsAutoIncrement = true; // Autoincrement field
        $this->IdInscription->IsPrimaryKey = true; // Primary key field
        $this->IdInscription->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->IdInscription->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->IdEleve->Lookup = new Lookup('IdEleve', 'eleve', false, 'IdEleve', ["MatricEleve","","",""], '', '', [], [], [], [], [], [], '', '', "`MatricEleve`");
                break;
            default:
                $this->IdEleve->Lookup = new Lookup('IdEleve', 'eleve', false, 'IdEleve', ["MatricEleve","","",""], '', '', [], [], [], [], [], [], '', '', "`MatricEleve`");
                break;
        ...
        $this->IdEleve->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['IdEleve'] = &$this->IdEleve;

        // IdClasse
        $this->IdClasse = new DbField(
            $this, // Table
            'x_IdClasse', // Variable name
            'IdClasse', // Name
            '`IdClasse`', // Expression
            '`IdClasse`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`IdClasse`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->IdClasse->InputTextType = "text";
        switch ($CurrentLanguage) {
            case "en-US":
                $this->IdClasse->Lookup = new Lookup('IdClasse', 'classe', false, 'IdClasse', ["NomClasse","","",""], '', '', [], [], [], [], [], [], '', '', "`NomClasse`");
                break;
            default:
                $this->IdClasse->Lookup = new Lookup('IdClasse', 'classe', false, 'IdClasse', ["NomClasse","","",""], '', '', [], [], [], [], [], [], '', '', "`NomClasse`");
                break;
        }
        $this->IdClasse->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->IdClasse->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->IdAnnee->Lookup = new Lookup('IdAnnee', 'classe', false, 'IdClasse', ["NomClasse","","",""], '', '', [], [], [], [], [], [], '', '', "`NomClasse`");
                break;
            default:
                $this->IdAnnee->Lookup = new Lookup('IdAnnee', 'classe', false, 'IdClasse', ["NomClasse","","",""], '', '', [], [], [], [], [], [], '', '', "`NomClasse`");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "inscription";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->IdInscription->setDbValue($conn->lastInsertId());
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['IdInscription']) && !EmptyValue($this->IdInscription->CurrentValue)) {
                $rs['IdInscription'] = $this->IdInscription->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('IdInscription', $rs)) {
                AddFilter($where, QuotedName('IdInscription', $this->Dbid) . '=' . QuotedValue($rs['IdInscription'], $this->IdInscription->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->IdInscription->CurrentValue : $this->IdInscription->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->IdInscription->CurrentValue = $keys[0];
            } else {
                $this->IdInscription->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('IdInscription', $row) ? $row['IdInscription'] : null;
        } else {
            $val = !EmptyValue($this->IdInscription->OldValue) && !$current ? $this->IdInscription->OldValue : $this->IdInscription->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@IdInscription@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InscriptionList");
    }
    ...
    {
        global $Language;
        if ($pageName == "InscriptionView") {
            return $Language->phrase("View");
        } elseif ($pageName == "InscriptionEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "InscriptionAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InscriptionView", $parm);
        } else {
            $url = $this->keyUrl("InscriptionView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InscriptionEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InscriptionList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InscriptionAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InscriptionList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("InscriptionDelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"IdInscription\":" . JsonEncode($this->IdInscription->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->IdInscription->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->IdInscription->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("IdInscription") ?? Route("IdInscription")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                ...
                $this->IdInscription->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // IdInscription
        $this->IdInscription->setupEditAttributes();
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->IdInscription);
                    $doc->exportCaption($this->IdEleve);
                    $doc->exportCaption($this->IdClasse);
        ...
        while (!$recordset->EOF && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->IdInscription);
                        $doc->exportField($this->IdEleve);
                        $doc->exportField($this->IdClasse);
