...
class EleveList extends Eleve
{
    ...
    public $FormName = "felevelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "EleveList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "EleveAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "EleveDelete";
        $this->MultiUpdateUrl = "EleveUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->IdEleve->AdvancedSearch->toJson(), ","); // Field IdEleve
        $filterList = Concat($filterList, $this->MatricEleve->AdvancedSearch->toJson(), ","); // Field MatricEleve
        $filterList = Concat($filterList, $this->NomEleve->AdvancedSearch->toJson(), ","); // Field NomEleve
        $filterList = Concat($filterList, $this->PostnomElve->AdvancedSearch->toJson(), ","); // Field PostnomElve
        $filterList = Concat($filterList, $this->PrenomEleve->AdvancedSearch->toJson(), ","); // Field PrenomEleve
        $filterList = Concat($filterList, $this->SexeEleve->AdvancedSearch->toJson(), ","); // Field SexeEleve
        $filterList = Concat($filterList, $this->DatenaissanceEleve->AdvancedSearch->toJson(), ","); // Field DatenaissanceEleve
        $filterList = Concat($filterList, $this->LieunaissanceEleve->AdvancedSearch->toJson(), ","); // Field LieunaissanceEleve
        $filterList = Concat($filterList, $this->NomdupereElve->AdvancedSearch->toJson(), ","); // Field NomdupereElve
        $filterList = Concat($filterList, $this->NomdelamereEleve->AdvancedSearch->toJson(), ","); // Field NomdelamereEleve
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "felevesrch", $filters);
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->NomEleve;
        $searchFlds[] = &$this->PostnomElve;
        $searchFlds[] = &$this->PrenomEleve;
        $searchFlds[] = &$this->SexeEleve;
        $searchFlds[] = &$this->LieunaissanceEleve;
        $searchFlds[] = &$this->NomdupereElve;
        $searchFlds[] = &$this->NomdelamereEleve;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"eleve\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"eleve\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"eleve\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"felevelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"felevelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->IdEleve->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"eleve\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"felevelist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("IdEleve", $this->createColumnOption("IdEleve"));
            $option->add("MatricEleve", $this->createColumnOption("MatricEleve"));
            $option->add("NomEleve", $this->createColumnOption("NomEleve"));
            $option->add("PostnomElve", $this->createColumnOption("PostnomElve"));
            $option->add("PrenomEleve", $this->createColumnOption("PrenomEleve"));
            $option->add("SexeEleve", $this->createColumnOption("SexeEleve"));
            $option->add("DatenaissanceEleve", $this->createColumnOption("DatenaissanceEleve"));
            $option->add("LieunaissanceEleve", $this->createColumnOption("LieunaissanceEleve"));
            $option->add("NomdupereElve", $this->createColumnOption("NomdupereElve"));
            $option->add("NomdelamereEleve", $this->createColumnOption("NomdelamereEleve"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"felevesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"felevesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="felevelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_eleve", "data-rowtype" => ROWTYPE_ADD]);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"felevesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
