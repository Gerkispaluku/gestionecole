...
$annee_edit = NULL; // Initialize page object first

class cannee_edit extends cannee {
	...
	function Page_Main() {
		...
		if (@$_GET["IdAnnee"] <> "") {
			$this->IdAnnee->setQueryStringValue($_GET["IdAnnee"]);
		}
		...
		// Check if valid key
		if ($this->IdAnnee->CurrentValue == "") {
			$this->Page_Terminate("anneelist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("anneelist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "anneelist.php")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// LibelleAnnee
			$this->LibelleAnnee->SetDbValueDef($rsnew, $this->LibelleAnnee->CurrentValue, NULL, $this->LibelleAnnee->ReadOnly);

			// DatedebutAnnee
			$this->DatedebutAnnee->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->DatedebutAnnee->CurrentValue, 0), NULL, $this->DatedebutAnnee->ReadOnly);

			// DatefinAnnee
			$this->DatefinAnnee->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->DatefinAnnee->CurrentValue, 0), NULL, $this->DatefinAnnee->ReadOnly);
...
// Create page object
if (!isset($annee_edit)) $annee_edit = new cannee_edit();

// Page init
$annee_edit->Page_Init();

// Page main
$annee_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$annee_edit->Page_Render();
?>
...
var CurrentForm = fanneeedit = new ew_Form("fanneeedit", "edit");

// Validate form
fanneeedit.Validate = function() {
...
fanneeedit.Form_CustomValidate = 
...
fanneeedit.ValidateRequired = true;
<?php } else { ?>
fanneeedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $annee_edit->ShowPageHeader(); ?>
<?php
$annee_edit->ShowMessage();
?>
<form name="fanneeedit" id="fanneeedit" class="<?php echo $annee_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($annee_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $annee_edit->Token ?>">
<?php } ?>
...
<?php if ($annee->IdAnnee->Visible) { // IdAnnee ?>
	<div id="r_IdAnnee" class="form-group">
		<label id="elh_annee_IdAnnee" class="col-sm-2 control-label ewLabel"><?php echo $annee->IdAnnee->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $annee->IdAnnee->CellAttributes() ?>>
<span id="el_annee_IdAnnee">
<span<?php echo $annee->IdAnnee->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $annee->IdAnnee->EditValue ?></p></span>
</span>
<input type="hidden" data-table="annee" data-field="x_IdAnnee" name="x_IdAnnee" id="x_IdAnnee" value="<?php echo ew_HtmlEncode($annee->IdAnnee->CurrentValue) ?>">
<?php echo $annee->IdAnnee->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($annee->LibelleAnnee->Visible) { // LibelleAnnee ?>
	<div id="r_LibelleAnnee" class="form-group">
		<label id="elh_annee_LibelleAnnee" for="x_LibelleAnnee" class="col-sm-2 control-label ewLabel"><?php echo $annee->LibelleAnnee->FldCaption() ?></label>
...
<?php if ($annee->DatedebutAnnee->Visible) { // DatedebutAnnee ?>
	<div id="r_DatedebutAnnee" class="form-group">
		<label id="elh_annee_DatedebutAnnee" for="x_DatedebutAnnee" class="col-sm-2 control-label ewLabel"><?php echo $annee->DatedebutAnnee->FldCaption() ?></label>
...
<?php if ($annee->DatefinAnnee->Visible) { // DatefinAnnee ?>
	<div id="r_DatefinAnnee" class="form-group">
		<label id="elh_annee_DatefinAnnee" for="x_DatefinAnnee" class="col-sm-2 control-label ewLabel"><?php echo $annee->DatefinAnnee->FldCaption() ?></label>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $annee_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
