...
class cchangepwd extends cagent {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["agent"])) $GLOBALS["agent"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'agent');
		}
	...
	function Page_Main() {
		...
		if ($bPostBack && $bValidate) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$rsold = $rs->fields;
					if (IsPasswordReset() || ew_ComparePassword($rsold['Motdepasse'], $this->OldPassword)) {
						$bValidPwd = TRUE;
						if (!IsPasswordReset())
							$bValidPwd = $this->User_ChangePassword($rsold, $sUsername, $this->OldPassword, $this->NewPassword);
						if ($bValidPwd) {
							$rsnew = array('Motdepasse' => $this->NewPassword); // Change Password
							$rs->Close();
