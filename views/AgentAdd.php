...
$Page->showMessage();
?>
<form name="fagentadd" id="fagentadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<script>
loadjs.ready("fagentadd", function() {
    fagentadd.createAutoSuggest(Object.assign({"id":"x_MatricAgent","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->MatricAgent->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.agent.fields.MatricAgent.autoSuggestOptions));
});
</script>
...
<span id="el_agent_EtatCivilAgent">
    <select
        ...
        data-select2-id="fagentadd_x_EtatCivilAgent"
        <?php } ?>
        data-table="agent"
...
<script>
loadjs.ready("fagentadd", function() {
    var options = { name: "x_EtatCivilAgent", selectId: "fagentadd_x_EtatCivilAgent" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fagentadd.lists.EtatCivilAgent?.lookupOptions.length) {
        options.data = { id: "x_EtatCivilAgent", form: "fagentadd" };
    } else {
        options.ajax = { id: "x_EtatCivilAgent", form: "fagentadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group">
    <input type="password" name="x_Motsecret" id="x_Motsecret" autocomplete="new-password" data-field="x_Motsecret" size="30" placeholder="<?= HtmlEncode($Page->Motsecret->getPlaceHolder()) ?>"<?= $Page->Motsecret->editAttributes() ?> aria-describedby="x_Motsecret_help">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fagentadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fagentadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
