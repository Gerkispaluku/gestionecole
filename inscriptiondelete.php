...
$inscription_delete = NULL; // Initialize page object first

class cinscription_delete extends cinscription {
	...
	// Page object name
	var $PageObjName = 'inscription_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("inscriptionlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in inscription class, inscriptioninfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("inscriptionlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($inscription_delete)) $inscription_delete = new cinscription_delete();

// Page init
$inscription_delete->Page_Init();

// Page main
$inscription_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$inscription_delete->Page_Render();
?>
...
var CurrentForm = finscriptiondelete = new ew_Form("finscriptiondelete", "delete");

// Form_CustomValidate event
finscriptiondelete.Form_CustomValidate = 
...
finscriptiondelete.ValidateRequired = true;
<?php } else { ?>
finscriptiondelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
finscriptiondelete.Lists["x_IdEleve"] = {"LinkField":"x_IdEleve","Ajax":true,"AutoFill":false,"DisplayFields":["x_MatricEleve","x_NomEleve","x_PostnomElve","x_PrenomEleve"],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"eleve"};
finscriptiondelete.Lists["x_IdClasse"] = {"LinkField":"x_IdClasse","Ajax":true,"AutoFill":false,"DisplayFields":["x_NomClasse","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"classe"};
finscriptiondelete.Lists["x_IdAnnee"] = {"LinkField":"x_IdAnnee","Ajax":true,"AutoFill":false,"DisplayFields":["x_LibelleAnnee","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"annee"};
...
<?php $inscription_delete->ShowPageHeader(); ?>
<?php
$inscription_delete->ShowMessage();
?>
<form name="finscriptiondelete" id="finscriptiondelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($inscription_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $inscription_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="inscription">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($inscription_delete->RecKeys as $key) { ?>
...
<?php echo $inscription->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($inscription->IdInscription->Visible) { // IdInscription ?>
		<th><span id="elh_inscription_IdInscription" class="inscription_IdInscription"><?php echo $inscription->IdInscription->FldCaption() ?></span></th>
<?php } ?>
<?php if ($inscription->IdEleve->Visible) { // IdEleve ?>
		<th><span id="elh_inscription_IdEleve" class="inscription_IdEleve"><?php echo $inscription->IdEleve->FldCaption() ?></span></th>
<?php } ?>
<?php if ($inscription->IdClasse->Visible) { // IdClasse ?>
		<th><span id="elh_inscription_IdClasse" class="inscription_IdClasse"><?php echo $inscription->IdClasse->FldCaption() ?></span></th>
<?php } ?>
<?php if ($inscription->IdAnnee->Visible) { // IdAnnee ?>
		<th><span id="elh_inscription_IdAnnee" class="inscription_IdAnnee"><?php echo $inscription->IdAnnee->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$inscription_delete->RecCnt = 0;
$i = 0;
while (!$inscription_delete->Recordset->EOF) {
	$inscription_delete->RecCnt++;
	$inscription_delete->RowCnt++;

	// Set row properties
	$inscription->ResetAttrs();
	$inscription->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$inscription_delete->LoadRowValues($inscription_delete->Recordset);

	// Render row
	$inscription_delete->RenderRow();
?>
	<tr<?php echo $inscription->RowAttributes() ?>>
<?php if ($inscription->IdInscription->Visible) { // IdInscription ?>
		<td<?php echo $inscription->IdInscription->CellAttributes() ?>>
<span id="el<?php echo $inscription_delete->RowCnt ?>_inscription_IdInscription" class="inscription_IdInscription">
<span<?php echo $inscription->IdInscription->ViewAttributes() ?>>
<?php echo $inscription->IdInscription->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($inscription->IdEleve->Visible) { // IdEleve ?>
		<td<?php echo $inscription->IdEleve->CellAttributes() ?>>
<span id="el<?php echo $inscription_delete->RowCnt ?>_inscription_IdEleve" class="inscription_IdEleve">
<span<?php echo $inscription->IdEleve->ViewAttributes() ?>>
<?php echo $inscription->IdEleve->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($inscription->IdClasse->Visible) { // IdClasse ?>
		<td<?php echo $inscription->IdClasse->CellAttributes() ?>>
<span id="el<?php echo $inscription_delete->RowCnt ?>_inscription_IdClasse" class="inscription_IdClasse">
<span<?php echo $inscription->IdClasse->ViewAttributes() ?>>
<?php echo $inscription->IdClasse->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($inscription->IdAnnee->Visible) { // IdAnnee ?>
		<td<?php echo $inscription->IdAnnee->CellAttributes() ?>>
<span id="el<?php echo $inscription_delete->RowCnt ?>_inscription_IdAnnee" class="inscription_IdAnnee">
<span<?php echo $inscription->IdAnnee->ViewAttributes() ?>>
<?php echo $inscription->IdAnnee->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
	</tr>
<?php
	$inscription_delete->Recordset->MoveNext();
}
$inscription_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $inscription_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
finscriptiondelete.Init();
</script>
<?php
$inscription_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$inscription_delete->Page_Terminate();
?>
