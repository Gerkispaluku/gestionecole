...
$annee_add = NULL; // Initialize page object first

class cannee_add extends cannee {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["IdAnnee"] != "") {
				...
				$this->setKey("IdAnnee", $this->IdAnnee->CurrentValue); // Set up key
			} else {
				$this->setKey("IdAnnee", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "anneeview.php")
...
// Create page object
if (!isset($annee_add)) $annee_add = new cannee_add();
...
var CurrentForm = fanneeadd = new ew_Form("fanneeadd", "add");

// Validate form
fanneeadd.Validate = function() {
...
fanneeadd.ValidateRequired = true;
<?php } else { ?>
fanneeadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $annee_add->ShowPageHeader(); ?>
<?php
$annee_add->ShowMessage();
?>
<form name="fanneeadd" id="fanneeadd" class="<?php echo $annee_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($annee_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $annee_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $annee_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
